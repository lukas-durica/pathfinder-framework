class_name AStarMAPDOrientedCppWrapper extends GridBasedAlgorithm

#var astar_cpp : = AStarCustom.new()
var astar_cpp : = AStarMAPDOrientedCpp.new()
func initialize(grd):
	astar_cpp.actual_time_step = 15
...
func find_solution(starts_and_goals : Array):
	...
	for sag in starts_and_goals:
		var start = sag.start
		var goal = sag.goal
		paths.push_back(astar_cpp.find_solution(Quat(start.x, start.y, 29.0, HORIZONTAL), 
				Quat(goal.x, goal.y, 0.0, VERTICAL), false))
	return paths
func clear():
